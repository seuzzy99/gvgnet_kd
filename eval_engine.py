...
from code.utils.metric import AverageMeter, multi_hot_targets, to_numpy, to_torch, auc, argmax_pts, L2_dist
from code.utils.visualize import draw_visualization, normed2original


def validate(cfg, module_list, data_loader, writer, epoch, ix_to_token, logger, save_ids=None, prefix='Val'):
    ...
    with torch.no_grad():
        ...
        for idx, (ref, image, image_gaze, face, head_img, mask, box, gt_box, mask_id, info, gaze_label, gaze_pts) in enumerate(data_loader):
            ...
            # MCN部分
            box, mask = model_mcn(image, ref, trans_feat_s)
            ...
            for i, mask_pred in enumerate(mask):
                if writer is not None and save_ids is not None and idx * cfg["train"]["batch_size"] + i in save_ids:
                    ...
                    sent = ' '.join(words)
                    box = box.view(box.shape[0], -1) * cfg["dataset"]["input_shape"][0]
                    ...
                    det_image = draw_visualization(
                        image=normed2original(image[i], [0., 0., 0.], [1., 1., 1.]),
                        sent=sent,
                        # pred_box=pred_box_vis[i].cpu().numpy(),
                        pred_box=pred_box_vis[i],
                        gt_box=box[i].cpu().numpy()
                    )

                    # cv2.imshow("out", det_image)
                    # cv2.waitKey(0)
                    # mask_img = (mask[i, None]*255).astype(np.uint8).transpose((1, 2, 0))
                    # cv2.imwrite('../output/seg/' + str(idx * cfg.train.batch_size+i) + '.jpg', mask_img)
                    # cv2.imwrite('../output/det/' + str(idx * cfg.train.batch_size + i) + '.jpg', det_image)
                ...
                # 读取mask图像
                mask_gt = cv2.imread(os.path.join('./data/masks/test/{}.png'.format(mask_id[i])), cv2.IMREAD_UNCHANGED)
