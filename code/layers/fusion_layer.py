...
class SimpleFusion(nn.Module):
    def __init__(self, v_planes=1024, q_planes=512, out_planes=1024):
        super().__init__()
    ...
    def forward(self, x, y, gaze):
        # input
        # x.shape: [N, 1024, 13, 13]
        # y.shape: [N, 512]
        # gaze.shape: [N, 512, 7, 7]

        x = self.v_proj(x)  # [N, 1024, 13, 13]
        gaze = self.gaze_proj(gaze)  # [N, 512, 7, 7] ---> [N, 512]
        gaze = gaze.squeeze(2).squeeze(2)
        gaze = self.gaze_fc(gaze)
        y_cat = torch.cat((y, gaze), dim=1)  # [N, 1024]
        y = self.q_proj(y_cat)  # [N, 1024]
        y = y.unsqueeze(2).unsqueeze(2)
        y = torch.Tensor.repeat(y, (1, 1, 13, 13))  # [N, 1024, 13, 13]
        out = self.norm(x + y)  # [N, 1024, 13, 13]
...
class ImageFusion(nn.Module):
    ...
    def forward(self, x, y, gaze):
        ...
        x = self.v_proj(x)  # [N, 1024, 13, 13]
        gaze = self.gaze_proj(gaze)  # [N, 512, 7, 7] ---> [N, 1024, 13, 13]
        gaze = torch.Tensor.repeat(gaze, (1, 1, 13, 13))
        gaze_x = self.norm_1(gaze * x)
        y = self.q_proj(y)  # [N, 1024]
        y = y.unsqueeze(2).unsqueeze(2)
        y = torch.Tensor.repeat(y, (1, 1, 13, 13))  # [N, 1024, 13, 13]
        out = torch.cat((gaze_x, y), dim=1)
        out = self.norm_2(out)  # [N, 1024, 13, 13]
...
if __name__ == '__main__':
    ...
    x = torch.randn(2, 1024, 13, 13)
    y = torch.randn(2, 1024)
    gaze = torch.randn(2, 512, 7, 7)
    out = model(x, y, gaze)