...
from code.layers.sa_layer import SA, AttFlat
...
class LSTM_SA(nn.Module):
    ...
    def forward(self, ques_ix):
        ...
        # torch.Size([8, 1, 1, 3])  lang_feat_mask
        # torch.Size([8, 3, 300])  lang_feat
        # torch.Size([8, 3, 512])  lang_feat
        # torch.Size([8, 512])    flat_lang_feat
        lang_feat_mask = make_mask(ques_ix.unsqueeze(2))
        lang_feat = self.embedding(ques_ix)
        lang_feat, _ = self.lstm(lang_feat)
        # lang_feat = self.lstm_fc(lang_feat)
        #
        #
