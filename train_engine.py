...
def train_one_epoch(cfg, module_list, optimizer, scheduler, data_loader, writer, epoch, criterion_list):
    # module_list = [model_mcn, model_s, model_kd, model_t]
    # 冻结教师网络
    module_list[0].train()
    module_list[1].train()
    module_list[2].train()
    ...
    for idx, (ref, image, image_gaze, face, head_img, mask, box, gt_box, mask_id, info, gaze_label, gaze_pts) in enumerate(data_loader):
        ...
        # MCN部分
        loss, loss_det, loss_seg = model_mcn(image, ref, trans_feat_s, det_label=box, seg_label=mask)
...
def main(cfg):
    ...
    # MCN网络
    model_mcn = GVGNet_KD(train_set.pretrained_emb, train_set.token_size, cfg).cuda()
    ...
    criterion_list.append(criterion_cls)    # S和Label间的损失 MSE
    criterion_list.append(criterion_kd)     # 特征层对齐损失 MSE

    # 打印模型信息
    total_params = sum([param.nelement() for param in module_list.parameters()])
    trainable_params = sum([param.nelement() for param in trainable_list.parameters() if param.requires_grad])
    trainable_params_mcn = sum([param.nelement() for param in trainable_list[0].parameters() if param.requires_grad])
    trainable_params_gaze = sum([param.nelement() for param in trainable_list[1].parameters() if param.requires_grad])

    logger.info("Number of all params: %.2fM" % (total_params / 1e6))
    logger.info("Number of trainable params: %.2fM" % (trainable_params / 1e6))
    logger.info("Number of mcn params: %.2fM" % (trainable_params_mcn / 1e6))
    logger.info("Number of gaze params: %.2fM\n" % (trainable_params_gaze / 1e6))
    ...
    for epoch in range(start_epoch, cfg["train"]["epochs"]):
        ...
        box_ap, mask_ap = validate(cfg, module_list, val_loader, writer, epoch, val_set.ix_to_token, logger, save_ids=save_ids)
